...
query_dept = pandas.read_sql_query(
    'SELECT COUNT(DISTINCT "Department") FROM mystaff.allstaff', engine
)
total_depts = query_dept.iloc[0][0]
# print(total_depts)

# The number of employees per department
query_epd = pandas.read_sql_query(
    'SELECT "Department", COUNT("LastName") FROM mystaff.allstaff GROUP BY "Department"',
    engine,
)
# print(query_epd)
